...
| **Dual-spec flex** | The role-shortage procedure: ask dual-spec players to switch to their `Offspec` when a role target cannot be met from signups. Voluntary — the player may decline. Canonical rule: `rules/01-raid-compositions.md` → "Handling role shortages (dual-spec flex)". |
...
| **First line offspec** | A pre-committed dual-spec flex disposition: a player whose `rules/04-players.md` Notes column contains "first line offspec" is asked first (Tier 0). Voluntary — the player may still decline. Canonical rule: `rules/01-raid-compositions.md` → "Handling role shortages → Asking order → Tier 0" (also applies in "Handling role surpluses"). |
